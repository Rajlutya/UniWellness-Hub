...
exports.searchStudentsByName = async (req, res) => {
    try {
        let { search } = req.query; // Ensure this matches the name attribute in your form

        // Validate and sanitize the search input
        if (!search || typeof search !== 'string') {
            search = ''; // Provide a default value or handle the case based on your application's logic
        }

        const students = await Register.find({ name: { $regex: search, $options: 'i' } }, 'name email');
        res.render('students', { students });
    } catch (err) {
...
exports.getStudentProfileForCounselor = async (req, res) => {
    try {
        const studentId = req.params.id; // Assuming student ID is passed in the URL params
        ...
        // Render the studentsprofile.hbs template with student and supportRequests data
        res.render('studentsprofileforcounselor', {
            student,
            pastHistory: formattedSupportRequests // Use pastHistory instead of supportRequests
        });
    } catch (error) {
